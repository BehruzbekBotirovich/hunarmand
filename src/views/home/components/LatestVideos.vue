    ...
    <section class="bg-graylight py-16">
        <div class="container">
            ...
            <div class="grid grid-cols-2 gap-6">
                <!-- Главное видео -->
                <div class="col-span-1 relative aspect-[5/3] rounded-2xl overflow-hidden p-4 flex items-end">
                    ...
                    <div class="video__info-block">
                        ...
                        <p class="text-base text-graydark/75 truncate-text">
                            You’ve read all your free member-only stories, become a member to get unlimited access. Your
                            membership fee supports the voices you want to hear more from. </p>
                    </div>
                </div>

                <!-- Слайдер с видео-карточками -->
                <div class="col-span-1 relative top-blur">
                    <swiper :modules="[Navigation]" :slides-per-view="1.42" :space-between="16"
                        ...
                        <swiper-slide v-for="(group, index) in chunkedVideos" :key="index">
                            <div class="flex flex-col gap-4">
                                <div v-for="item in group" :key="item.id"
                                    class="flex gap-3 bg-white p-3 rounded-2xl shadow">
                                    <div class="w-2/5">
                                        <img :src="item.image" class="h-full object-cover aspect-square rounded-xl"
                                            alt="">
                                    </div>
                                    <div class="w-3/5 py-4">
                                        <h3 class="mb-4 text-lg font-semibold">{{ item.title }}</h3>
                                        <p class="text-graydark/75   truncate-6">{{ item.description }}</p>
                                    </div>
                                </div>
                            </div>
                        </swiper-slide>
...
const chunkedVideos = computed(() => {
    return videos.reduce((acc, curr, index) => {
...
onMounted(() => {
    console.log("Swiper navigation:", prevBtn.value, nextBtn.value);
});
</script>
...
.top-blur::before {
    ...
    right: -4%;
    top: 0;
    background: linear-gradient(270deg, #F5F5F5 5.21%, rgba(245, 245, 245, 0.3827) 64.58%, rgba(245, 245, 245, 0) 100%);
}
